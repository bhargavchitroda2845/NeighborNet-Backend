    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if action == "save_draft":
                ...
                # Only go to inreview if not already published
                if previous_status != BnsModel.STATUS_PUBLISHED:
                    status = BnsModel.STATUS_INREVIEW
                else:
                    status = previous_status
            elif action == "mark_as_sold":
                # Allow member to mark item as sold directly without admin review
                # Go directly to SOLD status - no need for admin review
                status = BnsModel.STATUS_SOLD
            else:
...
def member_marketplace_bids(request, pk):
    ...
    has_winner = any(bid.is_winner for bid in accepted_bids)
...
def member_marketplace_bid_action(request, pk, action):
    ...
    if bid.marketplace_item.status == BnsModel.STATUS_SOLD:
        return JsonResponse({"detail": "This item has been sold and no longer accepts bid changes"}, status=400)
    ...
    if action == "accept":
        ...
        # Check if already a winner - prevent changing winner
        if bid.is_winner:
            return JsonResponse({
                "success": True, 
                "message": "This bid is already marked as Winner.",
                "status": bid.status,
                "is_winner": bid.is_winner
            })
        
        # Check if there's already a winner for this item - prevent changing
        existing_winner = Bid.objects.filter(
        ...
        if existing_winner:
            return JsonResponse({
                "success": False, 
                "detail": "A winner has already been selected for this item. You cannot change the winner.",
                "existing_winner_id": existing_winner.id
            }, status=400)
        ...
        return JsonResponse({"detail": "Invalid action. Use 'accept', 'reject', or 'win'"}, status=400)
...
def _serialize_bns_item(item):
    ...
    # Get winning bid info if item is sold
    winning_bid_info = None
    if item.status == BnsModel.STATUS_SOLD:
        winning_bid = item.bids.filter(is_winner=True).first()
        if winning_bid:
            winner_profile = _serialize_member_profile(winning_bid.bidder)
            winning_bid_info = {
...
def api_listing_type_list(request):
    ...
    for key, label in BnsModel.LISTING_TYPE_CHOICES:
        # Count published items for each listing type
        published_count = BnsModel.objects.filter(
            listing_type=key,
            status=BnsModel.STATUS_PUBLISHED,
        ).count()
        # Count sold items for each listing type
        sold_count = BnsModel.objects.filter(
            listing_type=key,
            status=BnsModel.STATUS_SOLD,
        ).count()
        ...
        data.append(
            {
                ...
                "total_count": published_count + sold_count,
            }
        )
    return JsonResponse({"count": len(data), "results": data})


def api_sold_count(request):
    """API to get total count of sold items"""
    total_sold = BnsModel.objects.filter(status=BnsModel.STATUS_SOLD).count()
    return JsonResponse({"total_sold": total_sold})
...
def api_all_marketplace(request):
    ...
    status_filter_param = (request.GET.get("status") or "").strip().lower()
    
    # Determine which statuses to include based on filter
    if status_filter_param == "sold":
        # Only show sold items
        status_filter = BnsModel.STATUS_SOLD
    elif status_filter_param == "published":
        # Only show published items
        status_filter = BnsModel.STATUS_PUBLISHED
    elif status_filter_param:
        # Use custom status if provided
        status_filter = status_filter_param
    else:
        # Default: show both published AND sold items
        status_filter = None
    ...
    if status_filter:
        ...
        # Show both published and sold items by default
        base_qs = base_qs.filter(status__in=[BnsModel.STATUS_PUBLISHED, BnsModel.STATUS_SOLD])
    ...
    # Apply area filter separately - only when explicitly provided as a separate parameter
    # This prevents issues when 'search' or 'q' parameter is used
    explicit_area = (request.GET.get("area") or "").strip()
    if explicit_area:
        qs = qs.filter(area__icontains=explicit_area)

    # Apply search filter - searches in title, description, area and contact fields
    if search:
        qs = qs.filter(
    ...
    return JsonResponse(
        {
            ...
            "previous": prev_url,
            "filters": {
                "status": status_filter if status_filter else "published_and_sold",
...
def api_place_bid(request):
    ...
    # Check if item is sold - prevent bidding on sold items
    if item.status == BnsModel.STATUS_SOLD:
        return JsonResponse({"detail": "This item has been sold and no longer accepts bids"}, status=400)

    if item.status != BnsModel.STATUS_PUBLISHED:
        return JsonResponse({"detail": "This item is not available for bidding"}, status=400)
