...
def send_bid_accepted_email(bid):
    """Send email notification when a bid is accepted (not winner yet)"""
    ...
    subject = f"Your bid has been accepted for '{item.title}'"
    
    message = f"""
Dear {bidder.first_name or bidder.username},

Good news! Your bid has been accepted by the seller.

Listing Details:
- Title: {item.title}
- Your Bid Amount: Rs {bid.bid_amount}
- Your Message: {bid.message or 'N/A'}

The seller will contact you soon for further details.
...
NeighborNet Team
    ...
    try:
        ...
        print(f"Bid accepted email sent to {bidder_email} for bid {bid.id}")
        return True
...
def send_winner_email(bid):
    """Send email notification when a bid is marked as winner (after physical process is complete)"""
    from django.core.mail import send_mail
...
def member_marketplace_bid_action(request, pk, action):
    ...
    if action == "accept":
        ...
        # Also mark as winner so it shows in accepted bids
        bid.status = Bid.STATUS_ACCEPTED
        bid.is_winner = True
        bid.save()
        
        # Send email that bid is accepted (not winner yet)
        email_sent = send_bid_accepted_email(bid)
        
        return JsonResponse({
            "success": True, 
            "message": "Bid accepted! The bidder has been notified that their bid was accepted.", 
            "status": bid.status,
            "is_winner": bid.is_winner
        })
    elif action == "win":
        # Mark as winner (just updates the is_winner flag, no email sent yet)
...
def api_manage_bid(request, bid_id):
    ...
    if action == "accept":
        ...
        # Send email that bid is accepted
        email_sent = send_bid_accepted_email(bid)
        
        if email_sent:
            return JsonResponse({
                "success": True, 
                "message": "Bid accepted! Bidder has been notified.", 
                "status": bid.status,
