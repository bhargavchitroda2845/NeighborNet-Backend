...
class BnsModelAdmin(admin.ModelAdmin):
    ...
    list_filter = ("status", "listing_type", "bidding_enabled", "published_at", "created_at")
    list_per_page = 10
...
class BidAdmin(admin.ModelAdmin):
    ...
    def is_winner_badge(self, obj):
        if obj.is_winner:
            return format_html(
                '<span style="background:#6f42c1;color:#fff;padding:2px 8px;border-radius:999px;font-size:11px;">Winner</span>'
            )
        return "-"
    is_winner_badge.short_description = "Winner"
